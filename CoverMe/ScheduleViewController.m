...
//fuction to set UISwitch ENABLE as employee's SHIFTS
-(void)setUISwitch:(int) employeeNum
              time:(int) timeNum
      working:(bool) shift{
    if (employeeNum == 0){
        if (timeNum == 0)
            [self.open0 setEnabled:shift];
        else if (timeNum ==1)
            [self.noon0 setEnabled:shift];
        else
            [self.close0 setEnabled:shift];
    }
    else if (employeeNum == 1){
        if (timeNum == 0)
            [self.open1 setEnabled:shift];
        else if (timeNum ==1)
            [self.noon1 setEnabled:shift];
        else
            [self.close1 setEnabled:shift];
    }
    else{
        if (timeNum == 0)
            [self.open2 setEnabled:shift];
        else if (timeNum ==1)
            [self.noon2 setEnabled:shift];
        else
            [self.close2 setEnabled:shift];
    }
                                ...
                                withBlock:^(FIRDataSnapshot * _Nonnull snapshot)
     {
         ...
         for ( FIRDataSnapshot *child in snapshot.children) {
             ...
             for(int i =0; i < 3; i ++){
                 ...
                 FIRDatabaseReference *refEmployeeShift = [[[[[_ref child:@"users"] child:employeeUserName] child:@"shift"] child:dayIndex] child: timeIndex];
                 
                 [refEmployeeShift observeSingleEventOfType: FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
                     if(snapshot.exists){
                         NSString *shiftString = snapshot.value;
                         bool shift;
                         if ([shiftString isEqualToString:@"0"]) {
                             shift = false;
                         }
                         else shift = true;
                         [self setUISwitch:employeeNum time:i working:shift];
                     }
                 }];
             }
